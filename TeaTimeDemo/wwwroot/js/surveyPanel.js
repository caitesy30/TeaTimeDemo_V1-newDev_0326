...
window.onload = function () {
    ...
    document.getElementById('addImageButton').addEventListener('click',async function () {
        ...
        if (previewImage) {
            ...
            await block.addNewBlockAddImg(previewImage.src)
        }
        // 關閉畫布
        offcanvas.hide();
    });
...
class tblResize {
        ...
        return Math.max(
            ...this.rows.map(row =>
                Array.from(row.cells)
                    .filter(td => td.style.display !== 'none' && !td.hidden)
                    .reduce((sum, td) => sum + (td.colSpan || 1), 0)
            )
        );
    }

    getActualColIndex(cell) {
    ...
    initializeRatios() {
        ...
        this.columnRatios = Array.from(this.cols).map((col, i) => {
            ...
            if (isNaN(ratio)) console.warn(`columnRatios[${i}] = NaN`);
            return ratio;
        });


        // 確保比例加總為 1，避免因浮點誤差導致比例異常
        const totalRatio = this.columnRatios.reduce((sum, ratio) => sum + ratio, 0) || 1;
        this.columnRatios = this.columnRatios.map((ratio, i) => {
            const norm = ratio / totalRatio;
            if (isNaN(norm)) console.warn(`Normalized ratio NaN at column ${i}`);
            return norm;
        });
    ...
    createColgroup() {
        ...
        const currentColumnCount = this.getMaxColCount();
        console.warn("總共：" + currentColumnCount + "格td");

        if (colgroup) {
            if (colgroup.children.length !== currentColumnCount) {
                colgroup.remove(); // 欄位數不一致，移除重建
                colgroup = null;
            }
        }
        ...
        if (!colgroup) {
            ...
            for (let i = 0; i < currentColumnCount; i++) {
                ...
                const widthRatio = this.columnRatios[i] || (1 / currentColumnCount);
                if (isNaN(widthRatio)) console.warn(`createColgroup: columnRatios[${i}] is NaN`);
                col.style.width = `${widthRatio * 100}%`;
    ...
    resizeHandler = (e) => {
        ...
        if (this.resizingType === 'column') {
            ...
            if (
                this.resizingIndex < 0 ||
                this.resizingIndex >= this.columnRatios.length - 1
            ) {
                console.warn("寬度 index錯誤 : ", this.resizingIndex);
                return;
            }
            const currentStartRatio = this.startRatios[this.resizingIndex];
            const nextStartRatio = this.startRatios[this.resizingIndex + 1];

            if (isNaN(currentStartRatio) || isNaN(nextStartRatio)) {
                console.warn(`NaN 比例: startRatios[${this.resizingIndex}] = ${currentStartRatio}, [${this.resizingIndex + 1}] = ${nextStartRatio}`);
                return;
            }
            ...
            // 避免 NaN 擴散
            if (isNaN(newCurrentRatio) || isNaN(newNextRatio)) {
                console.warn(`NaN 結果: newCurrentRatio = ${newCurrentRatio}, newNextRatio = ${newNextRatio}`);
                return;
            }
            ...
            //console.log("第" + this.resizingIndex + "格")
    ...
    handleMouseDown = (e) => {
        ...
        if (cell) {
            ...
            if (this.resizingType === 'row') {
                ...
                var targetMB = ModuleDataFetcherMgr.GetTargetModuleBlock(cell);
                var targetGrid = ModuleDataFetcherMgr.GetTargetModuleBlock_TableGrid(targetMB);
                 targetGrid.querySelectorAll("col");

                const rowIndex = Array.from(this.table.rows).indexOf(currentRow);

                if (rowIndex !== -1) {
                    // 若該儲存格有 rowspan，應以最後一列作為 resizingIndex
                    const rowStart = rowIndex;
                    const rowEnd = rowStart + rowspan - 1;
                    this.startRatios = this.rowRatios.slice(rowStart, rowEnd + 1);
                    this.resizingIndex = rowEnd; // 將 index 設為最終列，使用者期待調整最後一列高度
                } else {
                    // fallback 情況
                    this.startRatios = [...this.rowRatios];
                }
            }
        }
        ...
        if (!this.startRatios) {
            this.startRatios = this.resizingType === 'column'
                ? [...this.columnRatios]
                : [...this.rowRatios];
        }
    ...
    createResizeAlert: function () {
        ...
        closeBtn.addEventListener('click', () => {
            if (canvas) {
                ...
                floating.merger.forEach(m => m.deactivate());
                floating.merger.forEach(m => m.history = []); // 清空 Undo 記錄
                let tblArea = document.querySelectorAll(
                    `
             .${ModuleBlockElementMgr.ModuleBlock_innerName} > .${ModuleBlockElementMgr.TableGridName} > tr > td > .${ModuleBlockElementMgr.TableFieldName} ,
             .${ModuleBlockElementMgr.ModuleBlock_innerName} > .${ModuleBlockElementMgr.TableGridName} > tbody > tr > td > .${ModuleBlockElementMgr.TableFieldName}
        ...
        mergeBtn.addEventListener('click', () => {
            ...
            if (Table.isMerging) {
                floating.merger = Array.from(tblList).map(table => new TableMerger(table));
                floating.merger.forEach(m => m.activate());
            }
            else {
                floating.merger.forEach(m => m.deactivate());
                floating.merger.forEach(m => m.history = []); // 清空 Undo 記錄
...
const block = {
    ...
    init: function () {
        ...
        document.addEventListener('keydown',async (event) => {
            ...
            if (event.key === '/') {
              await  ModuleBlockCreatMgr.addModuleBlock(true);
            }
            if (event.key === 'Escape'){
                block.fixFreeze();
            }
    ...
    addNewBlock:async function () {
        var newModuleBlock = await ModuleBlockCreatMgr.addModuleBlock();
    ...
    addNewBlockAddImg:async function (imgSrc) {
    ...
    showId: function () {
        ...
        modules.forEach(module => {
            ...
            var moduleId = module.getAttribute('id') || "未設定 ID";
