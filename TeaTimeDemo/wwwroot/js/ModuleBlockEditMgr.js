...
class ModuleBlockEditMgr {
    ...
    static async OpenModuleBlock_TableGrid(TargetModuleBlock, bool) {
        ...
        if (TableGrid) {
            ...
            if (bool) {
                await ModuleBlock_GridSetMgr.initGridTable_Table(TargetModuleBlock);

                await ModuleBlockEditMgr.OpenModuleBlock_TableGrid(TargetModuleBlock, bool);
            } else {
                //設定為關 不用新增
            }
    ...
    static toggleCollapse(TargetModuleBlock) {
        ...
        TargetModuleBlock.dataset.collapsed = isCollapsed;
    ...
    static updateLabelCollapseIcon(TargetModuleBlock, collapsed) {
        ...
        var children = ModuleDataFetcherMgr.getAllChilds(TargetModuleBlock);

        //如果是列表狀態 有小孩 小孩<1 跳過
  
        if (TargetModuleBlock.dataset.GridMode != "true" && ( !children || children.length < 1 )) {
...
class ModuleBlock_GridSetMgr {
    ...
    //------------------------------------------------------------------------
    static async creatGridTable_Table() {
        ...
        for (var i = 0; i < 2; i++) {
            ...
            for (var j = 0; j < 2; j++) {
                ...
                var newModuleBlock = await ModuleBlock_GridSetMgr.createTextAreaModuleBlock();
                _Field.appendChild(newModuleBlock);
                tr.appendChild(td);
            }
    ...
    static SetTextArea(targetModuleBlock) {
        ...
        ModuleBlock_GridSetMgr.setDivElementInnerHTML(`
         <textarea id="${targetModuleBlock.id}_textarea" rows="1";
             ...
             oninput="ModuleBlock_GridSetMgr.adjustHeight(this)"></textarea>`);
    ...
    static async initGridTable_Table(targetModuleBlock) {

        var GridTable_Table = await ModuleBlock_GridSetMgr.creatGridTable_Table();
        ModuleDataFetcherMgr.GetTargetModuleBlock_OnceTable(targetModuleBlock)
            .parentNode.insertBefore(GridTable_Table,
                ModuleDataFetcherMgr.GetTargetModuleBlock_OnceTable(targetModuleBlock).nextSibling );
    }
    static async createTextAreaModuleBlock() {
    ...
    static async SetModuleBlockSGrid(targetId, AddOrRemove, RowOrColumn, HeadOrFeet) {
        // 获取目标模块块
        ...
        if (!ModuleDataFetcherMgr.GetTargetModuleBlock_TableGrid(moduleBlock)) {
            await ModuleBlock_GridSetMgr.initGridTable_Table(moduleBlock);
        }
        ...
        if (RowOrColumn === "Row") {
            if (AddOrRemove === "Add") {
                ...
                for (let i = 0; i < colCount; i++) {
                    ...
                    _Field.appendChild(await ModuleBlock_GridSetMgr.createTextAreaModuleBlock());
