    ...
    public class SurveyAnsController : Controller
    {
        ...
        public IActionResult Index(string MtNum, string ProcessName, bool vali = false)
        {
            ...
            for (int i = 0; i < PageSelectionList.Count; i++)
            {
                ...
                int targetVersion = 0;
                var maxVersion = _unitOfWork.DbContext.HtmlSections                                   
                    .Where(h => h.DocumentExportId == DocumentExportId)                               
                    .Max(h => h.Version);

                //### [ 根據目標 DocumentExportId 找出最大的版本，然後取得該版本的 html] (版本1)                                                                                      
                //targetVersion = maxVersion;
                //### [根據目標 DocumentExportId 跟版本 找到該目標的html]  (版本2)  
                targetVersion = Version;
                //====================================================================================

                htmlSections = _unitOfWork.DbContext.HtmlSections                                
                    .Where(h => h.DocumentExportId == DocumentExportId && h.Version == targetVersion)       
                    .OrderBy(h => h.Id)                                                               
                    .Select(h => h.HtmlPart)                                                          
                    .ToList();                                                                        
