...
namespace TeaTimeDemo.Areas.Admin.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        public OrderController(IUnitOfWork unitOfWork, ApplicationDbContext db, IConfiguration configuration)
        {
        ...
        */
        ...
        public IActionResult Index(string status)   //NotesList
        {
            ...
            switch (status)
            {
                ...
                case "CAM製作中":
                    answeredIndexVM.SelectedNotes = _unitOfWork.AnsweredNotes.GetAll(ans => ans.status == "CAM製作中");
                    break;
                case "CAM審核中":
                    answeredIndexVM.SelectedNotes = _unitOfWork.AnsweredNotes.GetAll(ans => ans.status == "CAM審核中");
                    break;
                case "QA":
                    answeredIndexVM.SelectedNotes = _unitOfWork.AnsweredNotes.GetAll(ans => ans.status == "QA");
                    break;
                case "產品檢核中":
                    answeredIndexVM.SelectedNotes = _unitOfWork.AnsweredNotes.GetAll(ans => ans.status == "產品檢核中");
                    break;
                case "完成":
        ...
        public IActionResult NotesView(int Id)   //Notes
        {
            ...
            var sortOrder = new List<string> { "PNL", "內層", "外層", "防焊", "印字", "其它" };
            ///PNL、內、外、防焊、印字、其他
            answeredNotesVM.AllProcess = answeredNotesVM.AllProcess.OrderBy(pg =>
            {
                // 找到 ProcessName 中第一個匹配的關鍵字的位置，沒有找到就設為 int.MaxValue
                int index = sortOrder.FindIndex(keyword => pg.ProcessName.Contains(keyword));
        ...
        [HttpGet]
        public IActionResult GetNoteLogs(int id)
        {
            string mtNum = _unitOfWork.AnsweredNotes.GetFirstOrDefault(a => a.Id == id).MtNum;
            var logs = _unitOfWork.NotesModify.GetAll(n => n.MtNum == mtNum).Select(log => new
            {
                log.Status,
                log.Remark,
                log.JobNum,
                log.JobName,
                CompleteTime = log.CompleteTime?.ToString("yyyy-MM-dd HH:mm")
            }).ToList();
            return Json(logs);
        }

        [HttpGet]
        public IActionResult GetMtNum(int id)
        {
            var mtFromDb = _unitOfWork.AnsweredNotes.GetFirstOrDefault(a => a.Id == id);
            var mtNum = mtFromDb.MtNum;
            var userId = mtFromDb.JobNum;
            var userName = mtFromDb.JobName;
            return Ok(new {
                mtNum = mtNum,
                jobNum = userId,
                jobName = userName
        ...
        [HttpPost]
        public async Task<IActionResult> ValiAgreement(string content, string mtNum, int jobNum, string jobName)
        {

            var sharedPath = _configuration["NetworkShare:SharedPath"];
            var folderPath = "ValiAgreement";
            if (string.IsNullOrWhiteSpace(content))
                return BadRequest("Content cannot be empty.");

            string folder = Path.Combine(sharedPath, folderPath);
            if (!Directory.Exists(folder))
                Directory.CreateDirectory(folder);

            string filePath = Path.Combine(folder, $"{mtNum}_{jobNum}_{jobName}.txt");

            await System.IO.File.WriteAllTextAsync(filePath, content);

            return Ok(new { message = "儲存成功", path = filePath });
        }



        [HttpPost]
        public async Task<IActionResult> UploadPdf(IFormFile pdf, string mtNum, string text)
        {

            var sharedPath = _configuration["NetworkShare:SharedPath"];
            var folderPath = "CopyConsent";
            var combinedPath = Path.Combine(sharedPath, folderPath);
            if (!Directory.Exists(combinedPath))
            {
                Directory.CreateDirectory(combinedPath);
            }

            var pdfPath = Path.Combine(combinedPath, $"{mtNum}.pdf");
            var txtPath = Path.Combine(combinedPath, $"{mtNum}.txt");
            
            try 
            {
                if (pdf != null)
                {
                    using (var pdfFile = new FileStream(pdfPath, FileMode.Create))
                    {
                        await pdf.CopyToAsync(pdfFile);
                    }
                }
                var content = $"差異內容：{text ?? "(無)"}{Environment.NewLine}";
                await System.IO.File.WriteAllTextAsync(txtPath, content);
                //string consentFolder = _configuration["CopyConsentFolder"];
                return Ok(new { success = true, message = "已確認同意" });
            }
            catch(Exception ex)
            {
                return BadRequest(new { success = false, message = ex + "發生錯誤" });
            }
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult Duplicate(string mtNum, string newNum)
        {
            try
            {
                var dbNotes = _unitOfWork.AnsweredNotes.GetFirstOrDefault(a => a.MtNum == mtNum);
                var dbans = _unitOfWork.DbContext.MtNumAnswereds.Where(m => m.MtNum == mtNum);

                if (dbNotes == null)
                {
                    return Json(new { success = false, message = "找不到此料號" });
                }
                string currentUserId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value; // 取得當前使用者 ID
                ApplicationUser currentUser = _unitOfWork.ApplicationUser.GetFirstOrDefault(u => u.Id == currentUserId);
                var processList = JsonConvert.DeserializeObject<List<OptionSelection>>(dbNotes.OptionList);
                foreach (var option in processList)
                {
                    foreach (var page in option.PageList)
                    {
                        var selectedPage = _unitOfWork.DocumentExport.GetFirstOrDefault(d => d.Id == page.SurveyId);
                        if (selectedPage != null && selectedPage.Version > page.Version)
                        {
                            page.Version = selectedPage.Version;
                        }
                    }
                }

                foreach (var ans in dbans)
                {
                    MtNumAnswered mtNumAnswered = new MtNumAnswered
                    {
                        MtNum = newNum,
                        SurveyId = ans.SurveyId,
                        Version = ans.Version,
                        AnsweredJson = ans.AnsweredJson,
                        Images = ans.Images,
                    ...
                    _unitOfWork.DbContext.MtNumAnswereds.Add(mtNumAnswered);
                }


                string jsonString = JsonConvert.SerializeObject(processList);

                AnsweredNotes copy = new AnsweredNotes
                {
                    PcbCategoryId = dbNotes.PcbCategoryId,
                    MtNum = newNum,
                    OptionList = jsonString,
                    CreateTime = DateTime.Now,
                    CompleteTime = DateTime.Now,
                    ApplicationUser = currentUser,
                    JobName = currentUser?.Name,
                ...
                _unitOfWork.AnsweredNotes.Add(copy);
                _unitOfWork.Save();
        ...
        /// <returns></returns>
        [HttpGet]
        public IActionResult GetPdf(string mtNum)
        {
            var target = $@"\\prdnas\notes\CopyConsent\{mtNum}.pdf";
        ...
        [HttpGet]
        public IActionResult CheckExistNotes(string mtNum)
        {
            if (string.IsNullOrEmpty(mtNum))
            {
                return BadRequest(new { success = false, message="料號為空!"});
            }
            ...
            {
                bool exist = true;
                return Ok(new { success = true, exist });
            }
            else
            {
                bool exist = false;
